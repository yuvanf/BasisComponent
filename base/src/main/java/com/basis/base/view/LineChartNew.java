...
import com.basis.base.bean.ChartEntity;
import com.basis.base.utils.CalculateUtil;
import com.basis.base.utils.DensityUtil;
...
public class LineChartNew extends View {
     ...
     * 背景的颜色
     */
    private static final int BG_COLOR = Color.parseColor("#F7F7F7");
    /**
     * 视图的宽和高
     */
     ...
     */
    private Paint mBgPaint, mAxisPaint, mLinePaint, mTextPaint, mPointPaint, mHintPaint,mDataPaint;
    /**
     * 原点的半径
     */
     ...
     * y洲 显示单位（分数）
     */
    private String mLeftAxisUnit = "分数";
    /**
     * 两个点之间的距离
     */
    private int mSpace;
    ...
    private int mHintColor = Color.parseColor("#427DFF");
    ...
    private void init(Context context) {
        ...
        mSpace = DensityUtil.dip2px(getContext(), 90);
        mBottomMargin = DensityUtil.dip2px(getContext(), 60);
        mTopMargin = DensityUtil.dip2px(context, 60);
        mRightMargin = DensityUtil.dip2px(getContext(), 60);
        mLeftMargin = DensityUtil.dip2px(getContext(), 0);

        mScroller = new Scroller(context);
        mMaxVelocity = ViewConfiguration.get(context).getScaledMaximumFlingVelocity();

        mBgPaint = new Paint();
        mBgPaint.setColor(Color.parseColor("#A0F7F7F7"));
        ...
        mTextPaint.setColor(Color.parseColor("#6E6E6E"));

        mDataPaint = new Paint();
        mDataPaint.setAntiAlias(true);
        mDataPaint.setTextSize(DensityUtil.dip2px(getContext(), 20));
        mDataPaint.setColor(Color.parseColor("#427DFF"));

        mLinePaint = new Paint();
        mLinePaint.setAntiAlias(true);
        mLinePaint.setStrokeWidth(DensityUtil.dip2px(context, 2));
        mLinePaint.setColor(Color.parseColor("#427DFF"));
...
//        canvas.clipRect(mDrawArea.left, mDrawArea.top, mDrawArea.right, mDrawArea.bottom + mDrawArea.height());
        ...
        //左边Y轴的单位
        canvas.drawText(mLeftAxisUnit, mStartX-20, mTopMargin / 2 - 14, mTextPaint);
...
//        float lineStart = mStartX + mTextPaint.measureText(mData.get(0).getxLabel()) / 2 + 20;
    ...
    private void drawCircles(Canvas canvas) {
        for (int i = 0; i < mData.size(); i++) {
            mPointPaint.setColor(Color.parseColor("#427DFF"));
//            canvas.drawCircle(mLinePoints.get(i), (mStartY - mData.get(i).getyValue() * maxHeight / mMaxYDivisionValue) * percent, RADIUS, mPointPaint);
            if (mLinePoints.get(i).x >= mStartX && mLinePoints.get(i).x < (mTotalWidth - mLeftMargin * 2)) {
                ...
                if (mData.get(i).getyValue() > 0) {
                    canvas.drawCircle(mLinePoints.get(i).x, mLinePoints.get(i).y * percent, RADIUS, mPointPaint);
                    mPointPaint.setColor(Color.parseColor("#F7F7F7"));
                    canvas.drawCircle(mLinePoints.get(i).x, mLinePoints.get(i).y * percent, RADIUS-2, mPointPaint);
                    canvas.drawText(String.valueOf(mData.get(i).getyValue()), mLinePoints.get(i).x - 15,
                            mLinePoints.get(i).y * percent - 10, mDataPaint);
                }
